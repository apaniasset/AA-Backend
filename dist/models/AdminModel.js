...
export class AdminModel {
    ...
    static async findByEmailOrPhone(identifier) {
        const [rows] = await pool.query('SELECT * FROM admin WHERE email = ? OR phone = ?', [identifier, identifier]);
    ...
    static async storeResetOTP(id, otp) {
        ...
        await pool.query('UPDATE admin SET reset_password_otp = ?, reset_password_expires = ? WHERE id = ?', [otp, expires, id]);
    }
    /**
     * Verify Reset OTP
     */
    static async verifyOTP(identifier, otp) {
        const [rows] = await pool.query('SELECT * FROM admin WHERE (email = ? OR phone = ?) AND reset_password_otp = ? AND reset_password_expires > NOW()', [identifier, identifier, otp]);
        return rows.length > 0 ? rows[0] : null;
    }
    /**
     * Update Password
    ...
    static async updatePassword(id, hashedPassword) {
        await pool.query('UPDATE admin SET password = ?, reset_password_otp = NULL, reset_password_expires = NULL WHERE id = ?', [hashedPassword, id]);
    }
}
