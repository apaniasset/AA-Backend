...
export class AuthController {
    static async login(req: Request, res: Response) {
        try {
            ...
            const token = jwt.sign(
                { id: affiliate.id, role: 'affiliate' },
    ...
    static async forgotPassword(req: Request, res: Response) {
        try {
            ...
            if (!identifier) return ApiResponse.error(res, 'Email or phone is required', 400);

            const affiliate = await AffiliateModel.findByEmailOrPhone(identifier);
            if (!affiliate) return ApiResponse.error(res, 'Affiliate not found', 404);
    ...
    static async resetPassword(req: Request, res: Response) {
        try {
            ...
            if (!affiliate) return ApiResponse.error(res, 'Invalid or expired OTP', 401);
